...
router.get('/dowloader/fbdown', async (req, res, next) => {
	...
	if (!url ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter url"})  
...
router.get('/dowloader/igstory', async (req, res, next) => {
	...
	if (!username ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter username"})   
...
router.get('/textpro/pencil', async (req, res, next) => {
	...
	if (!text1 ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter text"})   
...
router.get('/textpro/glitch2', async (req, res, next) => {
	...
	if (!text1 ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter text"})   
	if (!text2 ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter text2"}) 
...
router.get('/soundoftext', async (req, res, next) => {
	...
	if (!text1 ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter text"})   
	if (!lan ) return res.json({ status : false, creator : `Aine`, message : "[!] sila letak format lang dengan betul cek web site https://soundoftext.com/docs untuk lihat code lang"})   
...
router.get('/maker/circle', async (req, res) => {
	...
	if (!text ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter url"})
...
router.get('/maker/darkness', async (req, res) => {
	...
	var no = req.query.no
	if (!text ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter url"})
	if (!no ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter no"})
...
router.get('/maker/resize', async (req, res) => {
	...
	if (!text ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter url"})
	if (!width ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter width"})
	if (!height ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter height"})
...
router.get('/maker/attp', async (req, res) => {
	...
	if (!text ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter text"})
...
router.get('/maker/emojimix', async (req, res, next) => {
	...
	if (!emoji1 ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter emoji1"})
	if (!emoji2 ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter emoji2"})  
...
router.get('/linkshort/tinyurl', async (req, res, next) => {
	...
	if (!link ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter link"})  

    var islink = isUrl(link)
	if (!islink ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter url sahaja"})  
...
router.get('/linkshort/tinyurlwithalias', async (req, res, next) => {
	...
	if (!link ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter link"})  
	if (!alias ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter alias"})  
...
router.get('/info/githubstalk', async (req, res, next) => {
	...
	if (!user ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter user"})  
...
router.get('/info/translate', async (req, res, next) => {
	...
	if (!text ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter text"})  
	if (!lang ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter lang.  boleh lihat list bahasa di https://cloud.google.com/translate/docs/languages"})  
...
router.get('/emoji/apple', async (req, res, next) => {
	...
	if (!emoji1 ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter emoji"})
...
router.get('/tools/ebase64', async (req, res, next) => {
	var text1 = req.query.text
	if (!text1 ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter text"})  
	if (text1.length > 2048) return res.json({ status : false, creator : `Aine`, message : "[!] Maximal 2.048 String!"})
...
router.get('/islamic/surah', async (req, res, next) => {
	...
	if (!text1 ) return res.json({ status : false, creator : `Aine`, message : "[!] masukan parameter no"})  
	alip.surah(text1)
.then((data) =>{ 
	if (!data ) return res.json(loghandler.notfound)
		res.json({
